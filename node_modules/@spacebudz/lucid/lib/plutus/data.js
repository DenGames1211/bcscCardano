...
export const Data = {
        ...
        if (typeof item === "string") {
          ...
          return Object.entries(item).map(([title, fields], subIndex)=>{
            ...
            return {
              ...
              fields: fields instanceof Array ? fields : Object.entries(fields).map(([title, value])=>{
                if (title[0] !== title[0].toLowerCase()) {
  ...
  /** Convert cbor encoded data to plutus data */ from: (raw, type)=>{
    function jsonToData(json) {
      ...
      if ("map" in json) {
        return new Map(json.map.map(({ k, v })=>[
            jsonToData(k),
            jsonToData(v)
          ]));
      }
  ...
  castFrom: (data, type)=>{
    ...
    const { shape, definitions } = schema.definitions ? schema : {
      shape: schema,
      definitions: {}
    };
    function castFromHelper(data, type) {
      ...
      const shapeType = (shape.anyOf ? "enum" : shape["$ref"] ? "$ref" : "") || shape.dataType;
      switch(shapeType){
        case "$ref":
          {
              ...
              case "object":
                {
                  ...
                  args instanceof Array ? args.map((item, index)=>castToHelper(item, enumEntry.fields[index])) : enumEntry.fields.map((entry)=>{
