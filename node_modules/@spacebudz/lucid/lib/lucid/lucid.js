...
export class Lucid {
  ...
  constructor({ provider, network, wallet } = {}){
    ...
    this.utils = {
      ...
      slotsToUnixTime: (slots)=>{
        return slotConfig.zeroTime + (slots - slotConfig.zeroSlot) * slotConfig.slotLength;
      }
    };
    if (wallet) {
  ...
  async fromInstructions(instructions, forceUtxoResolution) {
    ...
    const instructionSigner = new InstructionBuilder(this.network, protocolParameters, utxos, {
      address
    }).commit(resolvedInstructions);
    return new TxComplete(this, instructionSigner);
  }
  /** Signs a message. Expects the payload to be Hex encoded. */ newMessage(address, payload) {
    return new Message(this, address, payload);
  }
  /** Verify a message. Expects the payload to be Hex encoded. */ verifyMessage(address, payload, signedMessage) {
  ...
  /** Unit needs to be an NFT (or optionally the entire supply in one UTxO). */ utxoByUnit(unit) {
  ...
  /** Query CIP-0068 metadata for a specifc asset. */ async metadataOf(unit) {
   ...
   */ selectWalletFromPrivateKey(privateKey) {
    ...
    this.wallet = {
      ...
      sign: (instructionSigner)=>{
        return instructionSigner.signWithKey(privateKey).getPartialWitnessSet();
      },
  ...
  selectWalletFromApi(api) {
    ...
    this.wallet = {
      ...
      rewardAddress: async ()=>{
        ...
        const rewardAddress = rewardAddressRaw ? Addresses.inspect(rewardAddressRaw).address : null;
      ...
      getDelegation: async ()=>{
        ...
        return rewardAddress ? await this.delegationAt(rewardAddress) : {
          poolId: null,
          drep: null,
          rewards: 0n
        };
   ...
   */ selectReadOnlyWallet({ address, rewardAddress, utxos }) {
    ...
    this.wallet = {
      address: ()=>address,
      rewardAddress: ()=>{
        return rewardAddress ? rewardAddress : delegation ? this.utils.credentialToRewardAddress(delegation) : null;
      },
      getUtxos: async ()=>{
   ...
   */ selectWalletFromSeed(seed, options) {
    ...
    const delegationDetails = options?.addressType === "Enterprise" ? null : Crypto.seedToDetails(seed, options?.index || 0, "Delegation");
    const address = this.utils.credentialToAddress(paymentDetails.credential, delegationDetails?.credential);
    const rewardAddress = delegationDetails ? this.utils.credentialToRewardAddress(delegationDetails.credential) : null;
    ...
    this.wallet = {
      ...
      sign: (instructionSigner)=>{
        return instructionSigner.signWithSeed(seed, index).getPartialWitnessSet();
      },
      signMessage: (address, payload)=>{
        const { payment, delegation } = Addresses.inspect(address);
