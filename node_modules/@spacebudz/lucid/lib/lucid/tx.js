...
export class Tx {
  ...
  /** Read data from utxos. These utxos are only referenced and not spent. */ readFrom(utxos) {
  ...
  /** Pay to a public key or native script address. */ payTo(address, assets) {
    this.tasks.push(()=>({
        type: "PayTo",
        address,
        assets
  ...
  /** Pay to a public key or native script address with datum or scriptRef. */ payToWithData(address, outputData, assets) {
    this.tasks.push(()=>{
      ...
      return {
        ...
        address,
        assets,
        datumVariant: Object.keys(datumVariant).length > 0 ? datumVariant : undefined,
        scriptRef
      };
    });
    return this;
  }
  /** Pay to a plutus script address with datum or scriptRef. */ payToContract(address, outputData, assets) {
    this.tasks.push(()=>{
      const { scriptRef, ...datumVariant } = typeof outputData === "string" ? {
        AsHash: outputData
      } : outputData;
  ...
  /** Delegate to a stake pool or drep. */ delegateTo(rewardAddress, variant, redeemer) {
    this.tasks.push(()=>({
        type: "DelegateTo",
        delegation: {
          rewardAddress,
  ...
  /** Register a reward address in order to delegate to a pool and receive rewards. */ registerStake(rewardAddress) {
    this.tasks.push(()=>({
        type: "RegisterStake",
        rewardAddress
      }));
  ...
  /** Deregister a reward address. */ deregisterStake(rewardAddress, redeemer) {
    this.tasks.push(()=>({
  ...
  /** Register a stake pool. A pool deposit is required. The metadataUrl needs to be hosted already before making the registration. */ registerPool(params) {
    this.tasks.push(async ()=>{
      if (params.metadataUrl && !params.metadataHash) {
        const metadata = await fetch(params.metadataUrl).then((res)=>res.arrayBuffer());
  ...
  /** Update a stake pool. No pool deposit is required. The metadataUrl needs to be hosted already before making the update. */ updatePool(params) {
    this.tasks.push(async ()=>{
      if (params.metadataUrl && !params.metadataHash) {
        const metadata = await fetch(params.metadataUrl).then((res)=>res.arrayBuffer());
        const metadataHash = Hasher.hashWithBlake2b256(toHex(new Uint8Array(metadata)));
  ...
  withdraw(rewardAddress, amount, redeemer) {
    this.tasks.push(()=>{
      const rewards = typeof amount !== "undefined" ? Number(amount) : undefined;
      return {
        type: "Withdraw",
        withdrawal: {
          rewardAddress,
  ...
  /** Add a payment or stake key hash as a required signer of the transaction. */ addSigner(keyHash) {
    this.tasks.push(()=>({
        type: "AddSigner",
        keyHash
      }));
  ...
  /** Converts strings to bytes if prefixed with **'0x'**. */ attachMetadataWithConversion(label, metadata) {
    this.tasks.push(()=>({
  ...
  /** Explicitely set the network id in the transaction body. */ addNetworkId(id) {
    this.tasks.push(()=>({
        type: "AddNetworkId",
        id
      }));
  ...
  withoutCoinSelection() {
    this.tasks.push(()=>({
        type: "WithoutCoinSelection"
      }));
    return this;
  }
  /** Compose transactions. */ compose(tx) {
  ...
  async commit() {
    ...
    const instructionSigner = new InstructionBuilder(this.lucid.network, protocolParameters, utxos, {
      address
    }).commit(instructions);
      ...
      case "Withdraw":
        {
          ...
          if (typeof instruction.withdrawal.amount === "undefined") {
            instruction.withdrawal.amount = Number((await lucid.delegationAt(instruction.withdrawal.rewardAddress)).rewards);
          }
          return instruction;
        }
      case "AddSigner":
