import { decodeHex, encodeHex } from "@jsr/std__encoding/hex";
...
/** Convert a Hex encoded string to a Utf-8 encoded string. */ export function toText(hex) {
  return new TextDecoder().decode(fromHex(hex));
}
/** Convert a Utf-8 encoded string to a Hex encoded string. */ export function fromText(text) {
  return toHex(new TextEncoder().encode(text));
}
/** Padded number in Hex. */ function checksum(num) {
...
export function applyParamsToScript(params, plutusScript, type) {
  const p = Data.to(type ? Data.castTo(params, type instanceof Array ? Data.Tuple(type) : type) : params);
