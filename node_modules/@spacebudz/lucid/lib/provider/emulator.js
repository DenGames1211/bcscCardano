...
export class Emulator {
  ...
  constructor(accounts){
    ...
    const utxos = accounts.map(({ address, assets, outputData }, index)=>{
      ...
      const datumHash = "AsHash" in datumVariant ? Hasher.hashData(datumVariant.AsHash) : "Hash" in datumVariant ? datumVariant.Hash : undefined;
      const datum = "AsHash" in datumVariant ? datumVariant.AsHash : "Inline" in datumVariant ? datumVariant.Inline : undefined;
      return {
        txHash: GENESIS_HASH,
        outputIndex: index,
        address,
  ...
  getUtxos(addressOrCredential) {
    const ledger = this.state.getLedger();
    const matchesAddress = (utxo)=>typeof addressOrCredential === "string" ? utxo.address === addressOrCredential : addressOrCredential.hash === paymentCredentialOf(utxo.address).hash;
    return Promise.resolve(ledger.filter((utxo)=>matchesAddress(utxo)));
  }
  getUtxosByOutRef(outRefs) {
    const ledger = this.state.getLedger();
    return Promise.resolve(ledger.filter((utxo)=>outRefs.some((outRef)=>utxo.txHash === outRef.txHash && utxo.outputIndex === outRef.outputIndex)));
  }
