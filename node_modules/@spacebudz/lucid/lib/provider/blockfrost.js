...
export class Blockfrost {
  ...
  async getProtocolParameters() {
    ...
    return {
      ...
      costModels: Object.fromEntries(Object.entries(result.cost_models).map(([plutus, costModel])=>[
          plutus,
          Object.values(costModel)
        ])),
  ...
  async getUtxos(addressOrCredential) {
    const queryPredicate = (()=>{
      ...
      const credentialBech32 = addressOrCredential.type === "Key" ? Utils.encodeBech32("addr_vkh", addressOrCredential.hash) : Utils.encodeBech32("addr_vkh", addressOrCredential.hash); // should be 'script' (CIP-0005)
  ...
  async getUtxosByOutRef(outRefs) {
    ...
    const utxos = await Promise.all(queryHashes.map(async (txHash)=>{
      ...
      const utxosResult = result.outputs.map((// deno-lint-ignore no-explicit-any
      r)=>({
          ...r,
          tx_hash: txHash
        }));
  ...
  async getDelegation(rewardAddress) {
    ...
    const drep = result.drep_id ? result.drep_id.includes("abstain") ? "Abstain" : result.drep_id.includes("confidence") ? "NoConfidence" : {
      Id: result.drep_id
    } : null;
    return {
