...
export class Maestro {
  ...
  async getProtocolParameters() {
    ...
    const renameKeysAndSort = (obj, newKeys)=>{
      ...
      return Object.fromEntries(Object.entries(sorted).map(([plutus, costModel])=>[
          plutus,
  ...
  async getUtxosInternal(addressOrCredential, unit) {
    const queryPredicate = (()=>{
      ...
      credentialBech32Query += addressOrCredential.type === "Key" ? Utils.encodeBech32("addr_vkh", addressOrCredential.hash) : Utils.encodeBech32("addr_shared_vkh", addressOrCredential.hash);
  ...
  async getDelegation(rewardAddress) {
    ...
    return {
      ...
      drep: "- information not available -",
  ...
  maestroUtxoToUtxo(result) {
    return {
      ...
      scriptRef: result.reference_script ? result.reference_script.type == "native" ? undefined : {
        type: result.reference_script.type[0].toUpperCase() + result.reference_script.type.slice(1).replace("v", "V"),
