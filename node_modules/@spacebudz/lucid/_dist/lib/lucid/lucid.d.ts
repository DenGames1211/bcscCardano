...
export declare class Lucid {
  ...
  /** Signs a message. Expects the payload to be Hex encoded. */ newMessage(address: string, payload: string): Message;
  /** Verify a message. Expects the payload to be Hex encoded. */ verifyMessage(address: string, payload: string, signedMessage: SignedMessage): boolean;
  ...
  /** Unit needs to be an NFT (or optionally the entire supply in one UTxO). */ utxoByUnit(unit: string): Promise<Utxo>;
  ...
  /** Query CIP-0068 metadata for a specifc asset. */ metadataOf(unit: string): Promise<Json>;
  /**
   ...
   */ selectReadOnlyWallet({}: ReadOnlyWallet): Lucid;
  /**
